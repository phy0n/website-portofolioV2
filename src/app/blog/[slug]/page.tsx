...
const BlogDetailPage: React.FC = () => {
  ...
  const blog = blogsData.find((b: any) => b.slug === slug) as Blog | undefined;
        ...
        <div className="prose prose-invert max-w-none mb-10 xs:mb-12 sm:mb-16">
          {blog.content.split('\n').map((paragraph, index) => {
            if (paragraph.startsWith('## ')) {
              return (
                <h2 key={index} className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold mt-10 xs:mt-12 sm:mt-16 mb-4 xs:mb-5 sm:mb-6 text-white">
                  {paragraph.replace('## ', '')}
                </h2>
              );
            }
            if (paragraph.startsWith('### ')) {
              return (
                <h3 key={index} className="text-lg xs:text-xl sm:text-2xl font-bold mt-8 xs:mt-10 sm:mt-12 mb-3 xs:mb-4 text-white">
                  {paragraph.replace('### ', '')}
                </h3>
              );
            }
